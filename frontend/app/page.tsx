...
function urgency(days: number) {
  if (days < 0) {
    return { label: "Overdue", style: "bg-red-50 text-red-700 ring-red-200" };
  }

  if (days === 0) {
    return { label: "Due today", style: "bg-red-50 text-red-700 ring-red-200" };
  }
...
export default function Home() {
  ...
  const [deadline, setDeadline] = useState(""); // typed as dd/mm/yyyy
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    if (!Number.isInteger(assignmentId) || assignmentId <= 0) {
      setIdError("Enter a valid ID (a positive whole number)");
      return;
    }

    if (assignments.some((a) => a.id === assignmentId)) {
      setIdError(`ID ${assignmentId} already exists. Try ${nextId}.`);
      return;
    }
    ...
    try {
      const response = await fetch(`${API_URL}/add`, {
        ...
        body: JSON.stringify({
          ...
          deadline: toISO(deadline), // backend/MySQL expects yyyy-mm-dd
          status,
        }),
      });
      ...
      setToast({
        type: "error",
        text: "Failed to add assignment. Is the server running?",
      });
    } finally {
      setSubmitting(false);
    }
  ...
  // Next free ID (highest existing + 1), shown as a hint in the form
  ...
  const visible = assignments
    ...
    .sort(
      (a, b) =>
        ...
        toISO(a.deadline).localeCompare(toISO(b.deadline))
    );
            ...
            {!loading && !loadError && visible.length > 0 && (
              <ul className="space-y-3">
                {visible.map((assignment) => {
                  ...
                  const badge = urgency(daysLeft(assignment.deadline));
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    value={deadline}
                    onChange={(e) => {
                      setDeadline(maskDate(e.target.value));
                      setDateError("");
                    }}
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      if (e.target.value) {
                        setDeadline(formatDate(e.target.value));
                        setDateError("");
                      }
                    }}
